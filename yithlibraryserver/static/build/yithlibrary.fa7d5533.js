...
Plugin.call($spy,data)})})}(jQuery);(function($){"use strict";$.fn.banner=function(options){return this.each(function(){var $source=$(this).find('.banner-source'),$preview=$(this).find('.banner-preview');$(this).find('.banner-selector button').click(function(){var src=$(this).attr("data-src"),code=$source.html();$preview.attr("src",src);code=code.replace(/(img src=")(.*)(" alt)/,"$1"+src+"$3");$source.html(code);});});};}(jQuery));(function($){"use strict";$.fn.checkAll=function(options){return this.each(function(){$(this).click(function(event){var isChecked=$(this).is(":checked"),$inputs=$("table input[type='checkbox']:not([disabled])");$inputs.prop("checked",isChecked);});});};}(jQuery));(function($){"use strict";$.fn.confirmForm=function(options){return this.each(function(){var target=$(this).data('target');$(this).click(function(event){$(target).submit();});});};}(jQuery));(function($){"use strict";$.fn.emailVerificationButton=function(options){return this.each(function(){$(this).click(function(event){var $button=$(this),$modal=$button.parents('.modal'),hideOnSuccess=$button.data('hide-on-success');event.preventDefault();if($button.hasClass("disabled")){return;}
$button.addClass("disabled");$.ajax({type:'POST',url:$button.attr("href"),data:{submit:'Send verification code'},dataType:'json',success:function(data,textStatus,jqXHR){if(data.status==='ok'){$modal.find(".alert-info").removeClass("hide");$(hideOnSuccess).addClass("hide");}else{$modal.find(".alert-error").removeClass("hide").find("strong").text(data.error);$modal.find(".alert-info").addClass("hide");$button.removeClass("disabled");}}});});});};}(jQuery));(function($){"use strict";var _gaq;if(window._gaq===undefined){_gaq=window._gaq=[];}
var addGoogleAnalyticsScript=function(code){var ga=document.createElement('script'),script=document.getElementsByTagName('script')[0];_gaq.push(['_setAccount',code]);_gaq.push(['_trackPageview']);ga.type='text/javascript';ga.async=true;ga.src=('https:'===document.location.protocol?'https://ssl':'http://www')+'.google-analytics.com/ga.js';script.parentNode.insertBefore(ga,script);};var GoogleAnalyticsForm=function(form,code){this.form=form;this.code=code;};GoogleAnalyticsForm.prototype.sendPreference=function(allow){var self=this,$form=$(this.form);$.ajax({type:$form.attr('method'),url:$form.attr('action'),data:allow,success:function(data){$form.parent('.alert').slideUp();if(data.allow){addGoogleAnalyticsScript(self.code);}}});};$.fn.googleAnalyticsPreferenceForm=function(options){return this.each(function(){var gaf=new GoogleAnalyticsForm(this,options.code);$(this).find('[name="yes"]').click(function(event){event.preventDefault();gaf.sendPreference({'yes':$(this).val()});}).end().find('[name="no"]').click(function(event){event.preventDefault();gaf.sendPreference({'no':$(this).val()});});});};$.fn.googleAnalyticsPreferenceForm.show=function(options){if(options.show){addGoogleAnalyticsScript(options.code);}};}(jQuery));(function($){"use strict";var getSearchParam=function(param){var items=window.location.search.substr(1).split("&"),index=0,item=[];for(index=0;index<items.length;index++){item=items[index].split("=");if(item[0]===param){return decodeURIComponent(item[1]);}}
return null;};$.persona=function(options){var currentUser=options.currentUser,currentProvider=options.currentProvider;if(!currentUser||currentProvider!=='persona'){currentUser=null;}
if(getSearchParam('force-persona-logout')==="true"){navigator.id.logout();}
if(options.loginSelector){$(options.loginSelector).click(function(event){event.preventDefault();navigator.id.request();});}
if(options.logoutSelector){$(options.logoutSelector).click(function(event){if(currentProvider==='persona'){event.preventDefault();navigator.id.logout();}});}
navigator.id.watch({loggedInUser:currentUser,onlogin:function(assertion){var form=["<form class='hide' ","action='"+options.loginUrl+"' ","method='post'>","<input type='hidden' name='assertion' ","value='"+assertion+"'/>",(options.nextUrl?"<input type='hidden' name='next_url' value='"+decodeURIComponent(options.nextUrl)+"' />":""),"</form>"].join("");$(form).appendTo("body").submit();},onlogout:function(){if(currentProvider==='persona'){window.location=options.logoutUrl;}}});};}(jQuery));(function($){"use strict";$.fn.wizard=function(options){return this.each(function(){$(this).find('.wizard-block .wizard-next').click(function(event){event.preventDefault();$(this).parents('.wizard-block').addClass("hidden").next('.wizard-block').removeClass('hidden');});});};}(jQuery));(function($,YITH){"use strict";$(document).ready(function(){$('.google-analytics-preference-form').googleAnalyticsPreferenceForm(YITH.ga);$.fn.googleAnalyticsPreferenceForm.show(YITH.ga);$('.banner').banner();$('.check-all').checkAll();$('.confirm-form').confirmForm();$('.btn-email-verification').emailVerificationButton();$('.wizard').wizard();$.persona(YITH.persona);$("[rel=popover]").popover({trigger:'hover'}).click(function(event){event.preventDefault();});$("[rel=tooltip]").tooltip().click(function(event){event.preventDefault();});});}(jQuery,YITH));